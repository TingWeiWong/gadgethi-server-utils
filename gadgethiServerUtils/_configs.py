...
class GServerConfigs:
	...
	def __init__(doday_flag=False, aws_flag=True):
		"""
		@params doday_flag: attach doday related server configs
		@params aws_flag: attach aws yaml fetch related configs
		"""
		configs = {}
		configs.update(basic_configs)

		if doday_flag:
			configs.update(doday_configs)

		if aws_flag:
			configs.update(aws_configs)
    ...
    @classmethods
    def check_current_configs_match_reqs(current_cfgs):
    	"""
    	...
    	if set(basic_configs.keys()).issubset(set(current_cfgs.keys())):
    		return True, input_defensive_copy

    	new_return_dict = copy.deepcopy(basic_configs)
